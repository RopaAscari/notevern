...
class CNTextInput extends Component {
  ...
  componentDidMount() {
   // console.log('MISC',this.props.miscellaneous)
    if (this.props.items) {
      this.textLength = 0;
  ...
  updateContent(content, item, index, itemNo = 0) {
    ...
    render() {
      ...
      const edit = !this.props.reading
      const misc = this.props.miscellaneous
   //   console.log(misc)
      ...
      return (
        ...
        >
          {
              _.map(items, item => (
                <CNStyledText key={item.id} style={item.styleList} item={item} text={item.text} />
              ))
            }
        </TextInput>
      );
