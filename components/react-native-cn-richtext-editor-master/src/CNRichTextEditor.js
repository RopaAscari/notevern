...
class CNRichTextEditor extends Component {
  ...
  insertVoiceRecording = (url, id = null, height = null, width = null) => {
  ...
  insertFileContent = (url, id = null, height = null, width = null) => {
    const {focusInputIndex} = this.state;
    const {value} = this.props;
    let index = focusInputIndex + 1;
    ...
    return (
      <View
        ...
        <CNTextInput
          ...
          items={input.content}
          reading={this.props.isReadMode}
          onSelectedStyleChanged={this.onSelectedStyleChanged}
          onSelectedTagChanged={this.onSelectedTagChanged}
          onEdit={this.props.editing}
  ...
  toggleAnimation = (audio) => {
    this.setState({animationStart: true});
    Animated.timing(this.state.animationValue, {
      toValue: 170,
      duration: Math.trunc(audio.getDuration()) * 1000,
      useNativeDriver: false,
    }).start(({finished}) => {
      this.setState({animationStart: false});
      this.setState({animationValue: new Animated.Value(0)});
    });
  };

  stopAnimation = (audio) => {
    const val = this.state.animationValue;
    Animated.timing(this.state.animationValue).stop(() => {
      this.setState({animationValue: new Animated.Value(val)});
    });
  };
  //const test = <Animated.View key={index} style={[styles(this.props.theme).audio,{position:'absolute',}, { width: this.state.animationValue, backgroundColor:this.state.animationStart > 0?'#555':'white' }]}/>

  setCurrentPlaying = (index) => {
    this.setState({currentPlayIndex: index});
  };

  renderVoiceRecording(item, index) {
    return (
      <AudioRecorder
        item={item}
        key={index}
        ...
        type={item.type}
        setCurrent={this.setCurrentPlaying}
  ...
  showPDF = (pdf) => {
    ...
    this.setState({currentPDF: pdf})
  }

  renderFile(file, index) {
    return (
      <TouchableOpacity onPress={()=>this.showPDF(file.url)} style={{ marginTop: 20, height: 200, width: 400, borderRadius: 35, alignItems:'center', justifyContent:'center', backgroundColor:'white', elevation:6}}>
        <View>
        <Pdf
          key={index}
          singlePage={true}
          source={{uri: file.url}}
          style={{height: 150, width: 400, borderRadius: 0,}}
        />
        </View>
      </TouchableOpacity>
    );
  ...
  renderImage(image, index) {
    ...
    if (
      ...
      (width === null && typeof height === 'undefined') ||
      height === null
    ) {
      width = 500;
      height = 200;
    ...
    // console.log('WID',myWidth , 'HEIGHT', myHeight)
    ...
    return (
      <View
        ...
        <TouchableWithoutFeedback
          onPress={() =>
            this.props.isReadMode ? null : this.onImageClicked(index)
          }>
          <ImageComponent
            style={{
              marginTop: 10,
              resizeMode: 'contain',
              width: image.isBase64 ? 400 : myWidth,
              height: image.isBase64 ? 730 : myHeight,
      ...
      <View
        ...
        <ScrollView
          ...
          <View
            ...
            {_.map(value, (item, index) => {
              ...
              if (item.component === 'voice') {
                return this.renderVoiceRecording(item, index);
              }
        ...
        <Modal
          ...
          style={{
            ...
            <MaterialCommunityIcons onPress={()=> this.setState({showPDFModal: false})} color="black" name="window-close" size={35} style={{position:'absolute', alignSelf:'flex-end', top: 10, right: 10, zIndex: 100}}/>
          <Pdf
